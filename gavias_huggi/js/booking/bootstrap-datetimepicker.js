! function(a, b) {
    ...
    if ("function" == typeof define && define.amd) define(["jquery", "moment"], b);
        ...
        if (!tbjQuery) throw new Error("bootstrap-datetimepicker requires jQuery to be loaded first");
        ...
        d = function(d, e) {
            var f, g = a.fn.datetimepicker.defaults,
                ...
                k = function() {
                    ...
                    if (i.id = c++, b.locale(i.options.language), i.date = b().add(i.options.minutesToAddForDefaultDateTime, 'minutes'), i.unset = !1, i.isInput = i.element.is("input"), i.component = !1, i.element.hasClass("input-group") && (i.component = i.element.find(0 === i.element.find(".datepickerbutton").size() ? '[class^="input-group-"]' : ".datepickerbutton")), i.format = i.options.format, f = b().add(i.options.minutesToAddForDefaultDateTime, 'minutes').localeData(), i.format || (i.format = i.options.pickDate ? f.longDateFormat("L") : "", i.options.pickDate && i.options.pickTime && (i.format += " "), i.format += i.options.pickTime ? f.longDateFormat("LT") : "", i.options.useSeconds && (-1 !== f.longDateFormat("LT").indexOf(" A") ? i.format = i.format.split(" A")[0] + ":ss A" : i.format += ":ss")), i.use24hours = i.format.toLowerCase().indexOf("a") < 0 && i.format.indexOf("h") < 0, i.component && (k = i.component.find("span")), i.options.pickTime && k && k.addClass(i.options.icons.time), i.options.pickDate && k && (k.removeClass(i.options.icons.time), k.addClass(i.options.icons.date)), i.options.widgetParent = "string" == typeof i.options.widgetParent && i.options.widgetParent || i.element.parents().filter(function () {
                ...
                q = function(a) {
                    ...
                    var c = a;
                    c || (c = l().val(), c && (i.date = b(c, i.format, i.options.useStrict)), i.date || (i.date = b().add(i.options.minutesToAddForDefaultDateTime, 'minutes'))), i.viewDate = b(i.date).startOf("month"), t(), x()
                },
                r = function() {
                    b.locale(i.options.language);
                    var c, d = a("<tr>"),
                        e = b.weekdaysMin();
                    if (i.options.calendarWeeks === !0 && d.append('<th class="cw">#</th>'), 0 === b().add(i.options.minutesToAddForDefaultDateTime, 'minutes').localeData()._week.dow)
                ...
                t = function() {
                    if (i.options.pickDate) {
                        ...
                        for (i.widget.find(".datepicker-days").find(".disabled").removeClass("disabled"), i.widget.find(".datepicker-months").find(".disabled").removeClass("disabled"), i.widget.find(".datepicker-years").find(".disabled").removeClass("disabled"), i.widget.find(".datepicker-days th:eq(1)").text(t[n] + " " + m), d = b(i.viewDate, i.format, i.options.useStrict).subtract(1, "months"), j = d.daysInMonth(), d.date(j).startOf("week"), (m === o && p >= n || o > m) && i.widget.find(".datepicker-days th:eq(0)").addClass("disabled"), (m === q && n >= r || m > q) && i.widget.find(".datepicker-days th:eq(2)").addClass("disabled"), e = b(d).add(42, "d"); d.isBefore(e);) {
                            if (d.weekday() === b().add(i.options.minutesToAddForDefaultDateTime, 'minutes').startOf("week").weekday() && (f = a("<tr>"), s.push(f), i.options.calendarWeeks === !0 && f.append('<td class="cw">' + d.week() + "</td>")), g = "", d.year() < m || d.year() === m && d.month() < n ? g += " old" : (d.year() > m || d.year() === m && d.month() > n) && (g += " new"), d.isSame(b({
                                    y: i.date.year(),
                                    M: i.date.month(),
                                    d: i.date.date()
                            })) && (g += " active"), (M(d, "day") || !N(d)) && (g += " disabled"), i.options.showToday === !0 && d.isSame(b().add(i.options.minutesToAddForDefaultDateTime, 'minutes'), "day") && (g += " today"), i.options.daysOfWeekDisabled)
                ...
                D = function(c) {
					...
					//special hack to set default time as 00:00 if time is not valid
                	if (!f.isValid()) {
		               i.setValue("00:00");
	                }
                },
                ...
                Q = function() {
                	var a = '<thead><tr><th class="prev">&lsaquo;</th><th colspan="' + (i.options.calendarWeeks ? "6" : "5") + '" class="picker-switch"></th><th class="next">&rsaquo;</th></tr></thead>',
                        ...
                        d = "";
                	return i.options.pickDate && i.options.pickTime ? (d = '<div class="bootstrap-datetimepicker-widget' + (i.options.sideBySide ? " timepicker-sbs" : "") + (i.use24hours ? " usetwentyfour" : "") + ' custom-dropdown-menu" style="z-index:9999 !important;">', d += i.options.sideBySide ? '<div class="row"><div class="col-sm-6 datepicker">' + c + '</div><div class="col-sm-6 timepicker">' + S.getTemplate() + "</div></div>" : '<ul class="list-unstyled"><li' + (i.options.collapse ? ' class="collapse in"' : "") + '><div class="datepicker">' + c + '</div></li><li class="picker-switch accordion-toggle"><a class="custom-btn" style="width:100%"><span class="' + i.options.icons.time + '"></span></a></li><li' + (i.options.collapse ? ' class="collapse"' : "") + '><div class="timepicker">' + S.getTemplate() + "</div></li></ul>", d += "</div>") : i.options.pickTime ? '<div class="bootstrap-datetimepicker-widget custom-dropdown-menu"><div class="timepicker">' + S.getTemplate() + "</div></div>" : '<div class="bootstrap-datetimepicker-widget custom-dropdown-menu"><div class="datepicker">' + c + "</div></div>"
                },
                R = {
                	modes: [{
                		clsName: "days",
            ...
            S.getTemplate = function() {
                return '<div class="timepicker-picker">' +
                    ...
                    '<tr>' +
                        '<td>' +
                            '<a href="#" class="custom-btn" data-action="incrementHours"><span class="' + i.options.icons.up + '"></span></a>' +
                        '</td>' +
                        '<td class="separator"></td>'
                        + '<td>' + (i.options.useMinutes ? '<a href="#" class="custom-btn" data-action="incrementMinutes"><span class="' + i.options.icons.up + '"></span></a>' : "") + "</td>"
                        + (i.options.useSeconds ? '<td class="separator"></td><td><a href="#" class="custom-btn" data-action="togglePeriod"><span class="' + i.options.icons.up + '"></span></a></td>' : "")
                        + ''
                        + (i.use24hours ? "" : '<td class="separator"></td><td><a href="#" class="custom-btn" data-action="toggleAmPm"><span class="icon-chevron-up"></span></a></td>') +
                    "</tr>" +
                    "<tr>" +
                        "<td>" + S.hourTemplate + '</td> ' +
                        '<td class="separator">:</td><td>' + (i.options.useMinutes ? S.minuteTemplate : '<span class="timepicker-minute">00</span>') + "</td> "
                        + (i.options.useSeconds ? '<td class="separator">:</td><td>' + S.secondTemplate + "</td>" : "")
                        + (i.use24hours ? "" : '<td class="separator">:</td><td><button type="button" class="custom-btn btn-link timepicker-period" data-action="togglePeriod">AM</button></td>') +
                    '</tr>' +
                    '<tr>' +
                        '<td><a href="#" class="custom-btn" data-action="decrementHours"><span class="' + i.options.icons.down + '"></span></a></td>' +
                        '<td class="separator"></td>' +
                        '<td>' + (i.options.useMinutes ? '<a href="#" class="custom-btn" data-action="decrementMinutes"><span class="' + i.options.icons.down + '"></span></a>' : "") + "</td>"
                        + (i.options.useSeconds ? '<td class="separator"></td><td><a href="#" class="custom-btn" data-action="decrementSeconds"><span class="' + i.options.icons.down + '"></span></a></td>' : "")
                        + (i.use24hours ? "" : '<td class="separator"></td><td><a href="#" class="custom-btn" data-action="toggleAmPm"><span class="icon-chevron-down"></span></a></td>') +
                    '</tr>' +
                    '<tr>' +
                        '<td colspan="5"><button type="button" class="custom-btn btn-primary btn-block btn-timepicker-close" data-action="closePicker">Set Time</button></td>' +
                    '</tr>' +
                    '</table>' +
                    '</div><div class="timepicker-hours" data-action="selectHour"><table class="table-condensed"></table></div><div class="timepicker-minutes" data-action="selectMinute"><table class="table-condensed"></table></div>' + (i.options.useSeconds ? '<div class="timepicker-seconds" data-action="selectSecond"><table class="table-condensed"></table></div>' : "")
            }, i.destroy = function() {
        		...
        		if (!l().prop("disabled")) {
        			if (i.options.useCurrent && "" === l().val()) {
        				if (1 !== i.options.minuteStepping) {
        				    var c = b().add(i.options.minutesToAddForDefaultDateTime, 'minutes'),
                                d = i.options.minuteStepping;
        					c.minutes(Math.round(c.minutes() / d) * d % 60).seconds(0), i.setValue(c.format(i.format))
        				} else i.setValue(b().add(i.options.minutesToAddForDefaultDateTime, 'minutes').format(i.format));
        				o("", a.type)
        			}
    ...
    a.fn.datetimepicker = function(b) {
		...
		daysOfWeekDisabled: [],
		widgetParent: !1,
		minutesToAddForDefaultDateTime: 0
	}
});